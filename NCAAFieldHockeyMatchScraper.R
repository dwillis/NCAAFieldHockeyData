...
urls <- read_csv("url_csvs/ncaa_womens_field_hockey_teamurls_2023.csv") %>% pull(3)
...
matchstatsfilename <- paste0("data/ncaa_womens_field_hockey_matchstats_", season, ".csv")
...
for (i in urls){
  ...
  opponentside <- matches %>% filter(opponent_info == "Defensive Totals") %>% select(-opponent_info) %>% select(-home_away) %>% rename_with(.cols = 7:26, function(x){paste0("defensive_", x)})
